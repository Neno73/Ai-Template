...
async function parseRoadmap(content: string): Promise<TaskStatus[]> {
  ...
  if (options.auto) {
    ...
    if (pendingTasks.length > 0) {
      ...
      const updatedContent = content.replace(
        `### [Task ${pendingTasks[0].id}] ${pendingTasks[0].title}`,
        `### [Task ${pendingTasks[0].id}] ${pendingTasks[0].title} (In Progress)`
      );
